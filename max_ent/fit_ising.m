...
for i = 1:max_iter
    ...
    %if converged
    if abs(tot_E_diff) < converge_thresh
    ...
    %if not converged    
        ...
        old_KL_div = sum ( log(P_emp ./ P_model) .* P_emp ); %compute KL divergence btwn empirical & old model
        new_KL_div = Inf; %initialize new KL divergence at infinity
        
        while new_KL_div > old_KL_div
            ...
            new_KL_div = sum ( log(P_emp ./ new_P_model) .* P_emp); %KL divergence btwn empirical and new model
