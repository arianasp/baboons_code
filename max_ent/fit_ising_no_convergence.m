function [ alpha_fit, beta_fit ] = fit_ising_no_convergence( data, weight, converge_thresh, max_iter, init_alphas, init_betas )
%Fits an ising model to the data.
%NOTE: THIS FUNCTION IGNORES THE CONVERGENCE THRESHOLD AND SIMPLY RUNS
%THROUGH THE NUMBER OF ITERATIONS GIVEN IN MAX_ITER
%INPUTS:
...
fprintf('Ran %d iterations \n',iter)
fprintf('KL = %e \n',curr_KL_div)
fprintf('dKL = %e \n',dKL)
warning('Model may or may not have converged! If you want to guarantee convergence, using fit_ising function instead!')
alpha_fit = coeffs(1:N);
beta_fit = tri_mat;
beta_fit(find(tri_mat==1))=coeffs((N+1):end);
for i = 1:N
