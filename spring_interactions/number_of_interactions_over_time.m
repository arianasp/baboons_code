function [ n_interactions_over_time ] = number_of_interactions_over_time( interactions_all, event_type, t_start, t_end, min_strength, min_disp )
%Get a timeseries of the number of events that occur between all pairs of 
%individuals, and store everything in a Tx1 vector
%INPUTS:
...
%       (defaults to 0)
%OUTPUTS:
%   n_interactions_over_time: [Tx1 vector] giving the number of ongoing
%       interactions (over all pairs of individuals) at a given time
...
n_interactions_over_time = zeros(t_end-t_start+1,1);
            ...
            if t0 >= t_start || tf <= t_end
                ...
                if strcmp(curr_type,event_type)
                    ...
                    if strength >= min_strength && disp >= min_disp
                        ...
                        n_interactions_over_time(start_idx:end_idx) = n_interactions_over_time(start_idx:end_idx) + 1;
                    end
                end
            end
        end
